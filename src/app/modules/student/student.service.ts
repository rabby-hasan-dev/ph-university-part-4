...
const getSingleStudentFromDB = async (id: string) => {
  ...
  const result = await Student.findById(id).populate('academicDepartment').populate('admissionSemester');
...
const deleteStudentFromDB = async (id: string) => {
  ...
  try {
    ...
    //  transaction-1
    const deletedStudent = await Student.findByIdAndUpdate(id, { isDelete: true }, { new: true, session });
    ...
    const deletedUser = await User.findByIdAndUpdate(userId, { isDelete: true }, { new: true, session });
...
const updateStudentFromDB = async (id: string, payload: Partial<TStudent>) => {
  ...
  const result = await Student.findByIdAndUpdate(id, modifiedData, { new: true, runValidators: true });
